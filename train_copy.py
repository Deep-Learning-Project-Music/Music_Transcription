...
def update_args(args): 
    args.logdir = 'runs/model/transcriber-' + datetime.now().strftime('%y%m%d-%H%M%S')
...
def train(logdir, device, iterations, resume_iteration, checkpoint_interval, train_on, batch_size, sequence_length,
    ...
    print("Testing.....", checkpoint_interval)
    ...
    best_note_f1 = 0
    #print("Eugenia is a sleepy sleepy!")
    count = 0
    for i, batch in zip(loop, cycle(loader)):
        #print("Sebby boo for the ladies...", count)
...
if __name__ == '__main__':
    ...
    if not args.no_log:
      wandb.login(key="Enter wandb key")
      wandb.init(project='low-latency-amt',
                 entity='Add entity here',
